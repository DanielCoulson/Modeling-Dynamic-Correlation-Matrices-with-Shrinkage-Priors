...
## STEP 1
##
## a_hat_t = R_hat_t^(-1/2) z_hat_t
##
## These are the empirical multivariate innovations that
## Fresoli-Ruiz resample.
############################################################
...
## STEP 2
##
## Generate one bootstrap sample from the fitted
## DCC-GARCH model using resampled empirical innovations.
############################################################

simulate_dcc_bootstrap <- function(
    fit,
    innovations,
    model = c("DCC", "aDCC")
) {
  ...
  pars <- extract_dcc_parameters(
    fit,
    model
  )
  
  original_data <- as.matrix(
    fit@model$modeldata$data
  )
  ...
  ##########################################################
  ## Resample complete multivariate innovations
  ##########################################################
  ...
  for (t in seq_len(TT)) {
    ...
    ## Correlated standardized residual
    z <- as.numeric(
      L %*% A_star[t, ]
    )
    ...
    if (t < TT) {
      ...
      ######################################################
      ## Additional asymmetric term for aDCC
      ######################################################
...
## STEP 3
##
## Use parameters estimated from Y* to filter the ORIGINAL
## return series Y.
##
## This produces one bootstrap correlation-score path.
############################################################

filter_original_score <- function(
    Y,
    bootstrap_fit,
    model = c("DCC", "aDCC")
) {
  ...
  ##########################################################
  ## IMPORTANT:
  ## use Qbar* estimated from the bootstrap sample
  ##########################################################
  ...
  for (t in seq_len(TT)) {
    ...
    ## Original-data residual under bootstrap parameters
    e <-
      Y[t, ] -
      pars$mu
...
############################################################
## Complete Fresoli-Ruiz bootstrap
############################################################

fit_dcc_with_uncertainty <- function(
    Y,
    model = c("DCC", "aDCC"),
    distribution = c("mvnorm", "mvt"),
    B = 200,
  ...
  innovations <-
    extract_bootstrap_innovations(fit)
  
  
  ##########################################################
  ## Bootstrap score paths
  ##########################################################
  ...
  for (b in seq_len(B)) {
    ...
    ########################################################
    ## Re-filter ORIGINAL Y with theta*
    ########################################################
    
    score_star <- try(
      filter_original_score(
        Y = Y,
        bootstrap_fit = fit_star,
        model = model
      ),
      silent = TRUE
    )
  ...
  successful <-
    colSums(is.finite(boot_scores)) == TT
  ...
  if (ncol(boot_scores) < 2) {
    stop(
      "Fewer than two bootstrap replications succeeded."
    )
  }
